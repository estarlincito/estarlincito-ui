...
const handleError = require('./handle-error.cjs');
...
const urlencoded = (obj) => {
    try {
        ...
        handleError.handleError('Invalid JSON string in body');
        return undefined;
    }
};
/**
 ...
 */
const apiFetch = async ({ url, body, headers, method, }) => {
    ...
    if (!res.ok) {
        handleError.handleError(`HTTP error! status: ${res.status}`);
    }
    return res;
};
